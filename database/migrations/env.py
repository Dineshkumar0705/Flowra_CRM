import sys
import os
from logging.config import fileConfig
from sqlalchemy import engine_from_config, pool
from alembic import context

# =========================
# 🧠 ADD PROJECT PATH
# =========================
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../../")))

# =========================
# 🔥 IMPORT CONFIG + MODELS
# =========================
from app.core.config import settings
from app.core.database import Base

# IMPORTANT: import all models here
from app.models import *  # ensures Alembic detects models


# =========================
# ⚙️ ALEMBIC CONFIG
# =========================
config = context.config

# Set DB URL dynamically
config.set_main_option("sqlalchemy.url", settings.database_url)

# Logging
if config.config_file_name is not None:
    fileConfig(config.config_file_name)


# =========================
# 📦 METADATA
# =========================
target_metadata = Base.metadata


# =========================
# 🔍 INCLUDE FILTER (OPTIONAL)
# =========================
def include_object(object, name, type_, reflected, compare_to):
    """
    Control what Alembic includes in migrations
    """
    # Skip internal tables if needed
    if type_ == "table" and name.startswith("pg_"):
        return False
    return True


# =========================
# 🧪 OFFLINE MIGRATION
# =========================
def run_migrations_offline():
    """
    Run migrations without DB connection
    """
    url = config.get_main_option("sqlalchemy.url")

    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
        compare_type=True,
        include_object=include_object,
    )

    with context.begin_transaction():
        context.run_migrations()


# =========================
# 🚀 ONLINE MIGRATION
# =========================
def run_migrations_online():
    """
    Run migrations with DB connection
    """

    connectable = engine_from_config(
        config.get_section(config.config_ini_section),
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )

    with connectable.connect() as connection:
        context.configure(
            connection=connection,
            target_metadata=target_metadata,

            # 🔥 IMPORTANT FLAGS
            compare_type=True,
            compare_server_default=True,
            include_object=include_object,

            # For batch mode (SQLite safety)
            render_as_batch=True,
        )

        with context.begin_transaction():
            context.run_migrations()


# =========================
# 🚦 EXECUTION
# =========================
if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()